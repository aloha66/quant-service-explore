...
- 涉及趋势研究的产品规划、阶段划分或功能探索时，阅读[研究大纲与原型阶段](docs/2026-09-21-a-share-trend-stock-selection-outline.md)。规划文档只写目标、内容范围、可体验闭环和待验证问题。
- 每个新功能都应能在原型中独立体验“输入 → 判断 → 操作 → 反馈 → 记录”，允许用历史回放或明确标记的模拟数据完成链路。
- **真正要做的内容决策，必须是通过原型推敲出来的结果。** 做哪些功能、范围多大、优先做什么，以及具体规则与交互，都应通过原型体验、比较和迭代后确定。规划大纲和讨论建议仅作为待验证方向，不能直接视为已确定的实施需求；原型中的临时方案须标为试验假设。
- 走势情境、异常变化及相应决策放到原型中逐步讨论；阶段文档保持内容大纲，不提前穷举情形、补齐状态机或固定规则。
- 形成结论时记录对应原型、体验发现和已确定的取舍，再同步相关需求与术语；未验证项保留为待探索。既有已生效的架构、数据契约和图表规范继续适用。
